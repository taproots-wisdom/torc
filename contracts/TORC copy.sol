...
contract TORC_Old is ERC20, ERC20Burnable, ERC20Pausable, Ownable, ERC20Permit {
    ...
    constructor(address initialOwner, address _uniswapV2RouterAddress, address _taxWallet, address _treasuryWallet, address _devWallet, address _marketingWallet, address _teamWallet)
        ERC20("Torc", "TORC")
	...
	function _distributeInitialBalances() internal {
		...
		_mint(address(this), _liquidityPoolAmount * 10 ** decimals()); 
	}
	...
	function initLP() external onlyOwner {
		...
		console.logAddress(address(_uniswapV2Router));
		console.log("WETH address: %s", address(_uniswapV2Router.WETH())); 
		console.log("Factory address: %s", address(_uniswapV2Router.factory()));		
		_approveRouter(_contractTokenBalance);
		// approve(address(this), type(uint256).max); 
		// approve(address(0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266), type(uint256).max); 
		address _uniLpAddr = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
		console.log("LP address: %s", _uniLpAddr);
	...
	function addLiquidity(uint256 _tokenAmount, uint256 _ethAmountWei) external onlyOwner {
		_addLiquidity(_tokenAmount, _ethAmountWei, false);
	}
	...
	function _addLiquidity(uint256 _tokenAmount, uint256 _ethAmountWei, bool autoburn) internal {
		...
		console.log("Adding liquidity with token amount: %s and eth amount: %s", _tokenAmount, _ethAmountWei); 
		console.log("LP token recipient: %s", lpTokenRecipient);
		console.log("TORC token address: %s", address(this));
		console.log("Router address: %s", address(_uniswapV2Router)); 
	...
	function taxSwapSettings(uint32 minValue, uint32 minDivider, uint32 maxValue, uint32 maxDivider) external onlyOwner {
		taxSwapMin = totalSupply() * minValue / minDivider;
		taxSwapMax = totalSupply() * maxValue / maxDivider;
		require(taxSwapMax >= taxSwapMin, "MinMax error");
		require(taxSwapMax > totalSupply() / 10000, "Upper threshold too low");
		require(taxSwapMax < totalSupply() * 2 / 100, "Upper threshold too high"); 
	}
	...
	function _swapTaxAndDistributeEth() private lockTaxSwap {
		...
		if ( _taxTokensAvailable >= taxSwapMin && tradingOpen ) {
			...
			if ( _taxTokensAvailable > 10** decimals()) { 
