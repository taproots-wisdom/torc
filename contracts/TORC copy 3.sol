...
contract TORC is Context, IERC20, Ownable {
    ...
    struct Investor {
        ...
        uint256 cut; // percentage cut (in basis points, 10000 = 100%)
        bool enabled;
    }

    Investor[4] public investors;
    uint256 public constant earthWalletCut = 50; // 0.5%
    address payable public earthWallet = payable(0xe82Bcb6d75Ec304D2447B587Dee01A0D5aB25785);
    address payable public ownerWallet = payable(0x9767a2B120614F526e923DAAF89843EC7C2292d7);

    // Log and Event Information for better troubleshooting.
    event Log(string, uint256);
    event AuditLog(string, address);
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity);
    event SwapTokensForETH(uint256 amountIn, address[] path);
    event InvestorWalletUpdated(uint index, address wallet);
    event InvestorCutUpdated(uint index, uint256 cut);
    event InvestorEnabledUpdated(uint index, bool enabled);
    ...
    constructor() {
        ...
        // Create Pair in the constructor, this may fail on some blockchains and can be done in a separate line if needed.
 ...
 address(0), "ERC20: approve to the zero address");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && overMinimumTokenBalance && swapAndLiquifyEnabled) {
            swapAndLiquify();
        }
    ...
    // swap for eth is to support the conversion of tokens to weth during swapAndLiquify this is a supporting function
    ...
    // ERC 20 standard transfer, only added if taking fees to count up the amount of fees for better tracking and split purpose.
    ...
    function excludeFromFee(address account) external onlyOwner {
        ...
        emit AuditLog("We have excluded the following wallet from fees:", account);
    }

    // Include wallet back in fees.
    function includeInFee(address account) external onlyOwner {
        _isExcludedFromFee[account] = false;
        emit AuditLog("We have included the following wallet in fees:", account);
    }

    // Automatic Swap Configuration.
    function setTokensToSwap(uint256 _minimumTokensBeforeSwap) external onlyOwner {
        require(_minimumTokensBeforeSwap >= 1, "You need to enter more than 1 token.");
        minimumTokensBeforeSwap = _minimumTokensBeforeSwap;
        emit Log("We have updated minimumTokensBeforeSwap to:", minimumTokensBeforeSwap);
    }
    ...
    function recoverETHfromContract() external onlyOwner {
        ...
        require(succ, "Transfer failed");
        emit AuditLog("We have recovered the stuck ETH from the contract.", treasuryWallet);
    }
    ...
    // New functions for managing investors

    function setInvestorWallet(uint index, address payable wallet) external onlyOwner {
        require(index < investors.length, "Invalid investor index");
        investors[index].wallet = wallet;
        emit InvestorWalletUpdated(index, wallet);
    }

    function setInvestorCut(uint index, uint256 cut) external onlyOwner {
        require(index < investors.length, "Invalid investor index");
        require(cut <= 10000, "Invalid cut percentage"); // Maximum 100%
        investors[index].cut = cut;
        emit InvestorCutUpdated(index, cut);
    }

    function setInvestorEnabled(uint index, bool enabled) external onlyOwner {
        require(index < investors.length, "Invalid investor index");
        investors[index].enabled = enabled;
        emit InvestorEnabledUpdated(index, enabled);
    }

    function distributeETH(uint256 amount) private {
        uint256 totalCut = 0;
        for (uint i = 0; i < investors.length; i++) {
            if (investors[i].enabled && investors[i].wallet != address(0)) {
                uint256 investorShare = (amount * investors[i].cut) / 10000;
                transferToAddressETH(investors[i].wallet, investorShare);
                totalCut += investorShare;
            }
        }
        // Earth Wallet
        uint256 earthWalletShare = (amount * earthWalletCut) / 10000;
        transferToAddressETH(earthWallet, earthWalletShare);
        totalCut += earthWalletShare;
        // Remaining ETH to owner wallet
        uint256 remaining = amount - totalCut;
        transferToAddressETH(ownerWallet, remaining);
    }
}
